...
out =);"
    ...
    "CPU(inM =, instruction =, reset =, writeM =, addressM =, pc =,
outM =,);"
...
out =);"
    ...
    "RAM16K(in =, load =, address =,
out =);"

    "RAM4K(in =, load =, address =,
out =);"

    "RAM512(in =, load =, address =,
out =);"

    "RAM64(in =, load =, address =,
out =);"

    "RAM8(in =, load =, address =,
out =);"
...
out =);"
    ...
    "Screen(in =, load =, address =,
out =);"

    "Xor(a =, b =,
out =);"))

(defvar rr/insert-chip-helm-source
  '((name . "insert chip")
    (candidates . rr/chip-api)
    (multiline)
    (action . (lambda (selected) (insert selected)))))

(defun rr/insert-chip ()
  (interactive)
  (helm :sources rr/insert-chip-helm-source
